...
namespace UniOption.Runtime
{
    public class Option<T> where T : class
    {
        ...
        public bool IsSomeAnd(Func<T, bool> predicate) => _content is not null && predicate(_content);
        ...
        public Option<T> Or(Option<T> orOption) => _content is null ? orOption : this;
        public IEnumerable<T> ToEnumerable(Option<T> option) => option._content is not null ? new[] { option._content } : Array.Empty<T>();
